...
const LANGUAGES: { name: string; phrase: string }[] = [
  ...
  { name: 'Armenian', phrase: 'Բարի գալուստ' },
...
export default function LanguageMarquee() {
  const rows = chunk(LANGUAGES, Math.ceil(LANGUAGES.length / 3));
  const headerRef = useScrollReveal<HTMLDivElement>();

  return (
    <section className="relative text-stone-50 py-28 overflow-hidden">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-stone-50/10 to-transparent" />

      <div ref={headerRef} className="scroll-reveal max-w-7xl mx-auto px-6 sm:px-10 mb-14">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          <div>
            ...
            <h2 className="mt-5 text-[36px] sm:text-[52px] font-semibold tracking-[-0.025em] leading-[1.04] max-w-2xl">
              Speak to <span className="text-shimmer animate-gradient-shift">every guest</span>,
              <br />
              in the language they call you in.
            </h2>
          </div>
          <p className="text-[14px] text-stone-400 max-w-md leading-[1.7]">
      ...
      <div className="space-y-4 marquee-mask">
        {rows.map((row, idx) => (
          <div
            key={idx}
            className={`flex gap-3 whitespace-nowrap ${idx % 2 === 0 ? 'animate-marquee' : 'animate-marquee-reverse'}`}
...
function LanguagePill({ name, phrase }: { name: string; phrase: string }) {
  return (
    <div className="group relative shrink-0 flex items-center gap-3 bg-stone-50/[0.04] hover:bg-stone-50/[0.08] border border-stone-50/10 hover:border-amber-300/40 rounded-2xl pl-3 pr-5 py-3 transition-all duration-300">
      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-400/30 to-rose-400/20 border border-amber-300/30 flex items-center justify-center text-[11px] font-semibold text-amber-100">
        {name.slice(0, 2).toUpperCase()}
      </div>
      <div>
        <div className="text-[13px] font-medium text-stone-100 leading-none">{name}</div>
        <div className="text-[12px] text-stone-400 mt-1 leading-none">{phrase}</div>
      </div>
    </div>
  );
}
