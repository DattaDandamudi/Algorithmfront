 ...
 * Analysis load card.
 *
 * It renders **bento tiles, not a block**: two 1×1 complications for the two
 * numbers any advice here is allowed to act on, then a span-2 tile for the
 * Banister state and the descriptive ratio beneath it. Size is the hierarchy
 * (DESIGN.md "Bento rules"), so the caller drops `<LoadGauge />` straight into
 * its `.hx-bento` and adds a `SectionHeader` on the ground above it.
 *
...
import { Tile, bandSoftBg, bandText } from '../../ui';
...
export interface LoadGaugeProps {
  ...
  /** Foot of the span-2 tile — the caller's "what this means" line. */
  meaning?: ReactNode;
}

export default function LoadGauge({ load, meaning }: LoadGaugeProps) {
  ...
  return (
    <>
      ...
      <Tile label="Acute load (7-day)" value={Math.round(load.acute7)} sub={`${fmt(Math.round(load.weeklyLoad), 0)} units logged this week`} />
      <Tile label="vs last week" value={formatPct(wow)} sub={wowWord} />

      <div className="hx-card hx-span-2 p-4 flex flex-col gap-4 overflow-hidden">
        {/* Banister state. */}
        ...
        <div className="border-t border-hx-border pt-3 flex flex-col gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="hx-label">Acute : chronic</span>
            <span
              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[13px] leading-[18px] font-medium ${bandSoftBg('neutral')} ${bandText('neutral')}`}
            >
              {load.acwr === null ? '—' : fmt(load.acwr, 2)}
            </span>
            <span className="text-[13px] leading-[18px] text-hx-text2">{acwrBandWord(load.acwrBand)}</span>
          </div>
