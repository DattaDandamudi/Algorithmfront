 ...
 * the status word beside every row, its own word legend, its hidden table and
 ...
 *
 * What this figure owns is the framing around it:
 *
 *  • the current week is **in progress**, and the dateline says so: a
...
export interface VolumeCardProps {
  /** `ctx.training.weeklySets` — this week's counts, for the lead. */
...
export default function VolumeCard({ weeklySets, weeks, onOpenTrain }: VolumeCardProps) {
  ...
  return (
    <TrendCard
      title="Weekly volume"
      caption={`This week from ${formatDateShort(weekStart)}, still in progress`}
      source={`Hard sets per muscle over ${weeks.length} weeks, filled by ink density against each muscle's landmark band.`}
    ...
    >
      <Lead label="This week so far" value={total} dp={Number.isInteger(total) ? 0 : 1} unit="hard sets" sub={belowLine ?? undefined} />
