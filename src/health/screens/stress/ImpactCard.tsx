  ...
  return (
    <li className="flex flex-col gap-1.5 py-3 first:pt-0 last:pb-0 border-b border-hx-border last:border-b-0">
      <div className="flex items-baseline justify-between gap-3 min-w-0">
        <span className="text-[15px] leading-[22px] font-medium text-hx-text min-w-0">{title}</span>
        <span className="hx-display text-[17px] leading-6 font-semibold text-hx-text shrink-0">{effectValueText(effect)}</span>
      </div>

      {bar && (
        <div className="relative h-7 w-full" role="img" aria-label={`${title}: ${effectValueText(effect)}, ${ciText(effect.lo95, effect.hi95)}`}>
          {/* track */}
          <div className="hx-well absolute inset-x-0 top-1/2 -translate-y-1/2 h-2" aria-hidden />
          {/* the 95 % interval */}
          <div
            className={`absolute top-1/2 -translate-y-1/2 h-2 rounded-full ${bar.crossesZero ? 'bg-hx-neutral' : bandBg(strength.tone)}`}
          ...
          <span className="absolute top-0 text-[12px] leading-4 text-hx-muted -translate-x-1/2" style={{ left: `${bar.zeroPct}%` }} aria-hidden>
            0
          </span>
          {/* the point estimate */}
          <div
      ...
      <p className={`text-[13px] leading-[18px] font-medium ${bandText(strength.tone)}`}>{strength.label}</p>
      {caveat && <p className="text-[13px] leading-[18px] text-hx-text2">{caveat}</p>}
      {shrink && <p className="text-[13px] leading-[18px] text-hx-muted">{shrink}</p>}
      {effect.confound && <p className="text-[13px] leading-[18px] text-hx-yellow">Confounded: {effect.confound}</p>}
    </li>
  );
}
...
export default function ImpactCard({ impact, max = 4 }: ImpactCardProps) {
  ...
  return (
    <TrendCard
      title="What moves your numbers"
      tile
      caption="The difference between your days with and without each behaviour"
    ...
    >
      ...
      <div className="rounded-ctl border border-hx-border bg-hx-card2/60 p-3">
        <p className="text-[13px] leading-[18px] text-hx-text2">
